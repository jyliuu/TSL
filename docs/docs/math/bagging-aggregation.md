...
<figure markdown="span">
  ...
  <figcaption markdown="span">Synthetic example — bagged <a href="../code/grid-tensor.md">GridTensors</a> at epoch 0 converging to two distinct backbone shapes for the same stage (two clusters of curves visible in the plot). This bimodality is a consequence of the <a href="model.md#identifiability-and-stability">sign-flip non-identifiability</a> of the two-tensor form: both orientations fit the residuals equally well, so different trees in the bag find different local optima. The align-then-filter step computes pairwise backbone similarity, anchors to a reference tree, and discards outlier trees before averaging — collapsing the two clusters into a single canonical representation. Reproduced by the <a href="../index.md#examples"><code>synthetic2.py</code></a> example script.</figcaption>
</figure>
